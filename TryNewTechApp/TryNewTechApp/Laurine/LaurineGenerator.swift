...
class Runtime {
    ...
    var localizationFilePathToRead: String!
    var localizationFilePathToWriteTo: String!
    var localizationFileHeaderPathToWriteTo: String?
    ...
    private func checkCLI() -> Bool {
        ...
        do {
            ...
            self.localizationFilePathToRead = inputFilePath.value!
            ...
            if let value = outputFilePath.value {
                self.localizationFilePathToWriteTo = value
    ...
    private func checkIO() -> Bool {
        ...
        if !FileManager.default.fileExists(atPath: self.localizationFilePathToRead) {
        ...
        if self.localizationExportStream == .File {
            ...
            _ = try? FileManager.default.removeItem(atPath: self.localizationFilePathToWriteTo)
            if !FileManager.default.createFile(atPath: self.localizationFilePathToWriteTo, contents: Data(), attributes: nil) {
            ...
            if self.localizationExportLanguage == .ObjC {
                ...
                let localizationFilePathToWriteToUrl = URL(fileURLWithPath: self.localizationFilePathToWriteTo)
                self.localizationFileHeaderPathToWriteTo = localizationFilePathToWriteToUrl.deletingPathExtension().appendingPathExtension("h").path
                
                // Remove file at path and replace it with new one
                _ = try? FileManager.default.removeItem(atPath: self.localizationFileHeaderPathToWriteTo!)
                if !FileManager.default.createFile(atPath: self.localizationFileHeaderPathToWriteTo!, contents: Data(), attributes: nil) {
    ...
    private func processOutput() {
        ...
        let localizationFilePathToReadUrl = URL(fileURLWithPath: self.localizationFilePathToRead)
        self.localizationCore = Localization(inputFile: localizationFilePathToReadUrl, delimiter: self.localizationDelimiter, autocapitalize: self.localizationAutocapitalize, table: self.localizationStringsTable)
        ...
        // or write output for objc, based on user configuration
            ...
            let localizationFilePathToWriteToUrl = URL(fileURLWithPath: self.localizationFilePathToWriteTo)
            let implementation = self.localizationCore.writerWithObjCImplementationWithFilename(filename: localizationFilePathToWriteToUrl.deletingPathExtension().lastPathComponent)
...
class StreamWriter {
    ...
    func writeToOutputFileAtPath(path: String, clearBuffer: Bool = true) {
        
        _ = try? self.outputBuffer.write(toFile: path, atomically: true, encoding: String.Encoding.utf8)
...
class TemplateFactory {
    ...
    class func templateForSwiftStaticVarWithName(name: String, key: String, table: String?, baseTranslation: String, contentLevel: Int) -> String {
        ...
        return TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "/// Base translation: \(baseTranslation)\n"
             + TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "public static var \(name) : String { return LocalizedString(\"\(key)\", \(tableName)comment: \"\") }\n"
    }
    ...
    class func templateForSwiftFuncWithName(name: String, key: String, table: String?, baseTranslation: String, methodHeader: String, params: String, contentLevel: Int) -> String {
        ...
        return TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "/// Base translation: \(baseTranslation)\n"
             ...
             + TemplateFactory.contentIndentForLevel(contentLevel: contentLevel + 1) + "return String(format: LocalizedString(\"\(key)\", \(tableName)comment: \"\"), \(params))\n"
             + TemplateFactory.contentIndentForLevel(contentLevel: contentLevel) + "}\n"
    }
